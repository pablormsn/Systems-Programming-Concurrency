...
public class Vista extends JPanel {
	...
	public Vista() {
		...
		this.add(BorderLayout.SOUTH, progreso);
	...
	public void establecerProgreso(int i) {
		progreso.setValue(i);
	}
	...
	public void actualizar(List<Float> lfs) {
		...
		for (int i = 0; i < lfs.size(); i++) {
			if (lfs.get(i) > 0) {
				contMayor++;
				this.anyadirListaMayores(String.format("%.02f", lfs.get(i)) + " ");
				if ((contMayor) % 5 == 0)
					this.anyadirListaMayores("\n");
			} else {
				contMenor++;
				this.anyadirListaMenores(String.format("%.02f", -lfs.get(i)) + " ");
				if ((contMenor) % 5 == 0)
