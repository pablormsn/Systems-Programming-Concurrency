...
int main(void) {
	...
	*/
	...
	//Lista_Extraer(&listaComponentes);
	for(int i = 0; i < 2; ++i){
	 //int vacia = Lista_Vacia(listaComponentes);
	 Adquirir_Componente(&codigo,texto);
	 Lista_Agregar(&listaComponentes,codigo,texto);
	 int num = Num_Elementos(listaComponentes);
	}

	Lista_Salvar(listaComponentes);

	Lista_Vaciar(&listaComponentes);
	Lista_Imprimir(listaComponentes);
	//Num_Elementos(listaComponentes);
	/*
